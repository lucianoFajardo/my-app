...
export default function CreateWithdrawalsPage() {
  ...
  const [selectedClient, setSelectedClient] =
    useState<DrawalsClientModel | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const data = await showWithdrawalsDataAction({
          from,
          to,
        ...
        throw new Error(
          `Error al cargar los clientes para los retiros programados: ${error}`,
        );
      } finally {
        setLoading(false);
      }
        ...
        <CardContent className="p-0">
          <div className="w-full overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {client.length > 0 ? (
                  client.map((currentClient) => {
                    const isWithdrawal =
                      currentClient.status ||
                      isWithdrawals.includes(currentClient.id_client);
